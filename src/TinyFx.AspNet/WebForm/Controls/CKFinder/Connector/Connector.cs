...
using System.Web.UI;
using System.Web;
using TinyFx.AspNet.WebForm.Controls.CKFinder;
using TinyFx.AspNet.WebForm.Controls;
...
namespace CKFinder.Connector
{
	...
	public class Connector : Page
	{
		...
		{
            ConfigFile = Activator.CreateInstance(Type.GetType(CKFinderControl.CKFINDER_CONFIG_DEFAULT)) as Settings.ConfigFile;
            ConfigFile.BaseUrl = ResolveClientUrl("~/upload/");
            var ckfinderId = Request.QueryString["id"];

            if (!string.IsNullOrEmpty(ckfinderId))
            {
                var cookie = Request.Cookies[CKFinderControl.CKFINDER_COOKIE_NAME];
                if (cookie != null)
                {
                    var cookieValue = cookie.Values.Get(ckfinderId);
                    if (!string.IsNullOrEmpty(cookieValue))
                    {
                        ...
                        if (!string.IsNullOrEmpty(values[0]))
                        {
                            ...
                            ConfigFile = Activator.CreateInstance(Type.GetType(typeName)) as Settings.ConfigFile;
                        }
                        if (values.Length > 1 && !string.IsNullOrEmpty(values[1]))
                            ConfigFile.BaseUrl = values[1];
                        if (values.Length > 2 && !string.IsNullOrEmpty(values[2]))
                            ConfigFile.AllowedExtensions = values[2].Split(',');
                        if (values.Length > 3 && !string.IsNullOrEmpty(values[3]))
                            ConfigFile.HideFolders = values[3].Split(',');

                    }
                }
            }
            this.Page.Controls.Add(ConfigFile);
