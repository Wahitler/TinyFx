...
(
   ...
   PrivParamsValue      varchar(250)  comment '功能和数据权限参数。
             类型-参数-是否有权限| 类型-参数-是否有权限
             如：ControlID-btnOk-true|ControlID-btnCancle-false
             
             ',
   MapUserID            varchar(20)  comment '映射的第三方平台用户编码
...
(
   ...
   KindID               varchar(50)  comment '来源表编码',
   Note                 varchar(255)  comment '描述',
   Status               tinyint not null default 0  comment '状态 0-无效 1-有效',
   RecDate              datetime not null default CURRENT_TIMESTAMP  comment '记录日期',
...
(
   AdminID              varchar(20) not null  comment '管理用户ID',
...
(
   LogID                bigint unsigned not null auto_increment  comment '日志主键',
   LogDate              datetime not null  comment '日志生成时间',
   Level                varchar(10) not null  comment '日志等级
             ...
             4-ERROR
             5-FATAL',
   Logger               varchar(50)  comment '日志记录类名称',
   Message              text  comment '日志消息',
   Exception            text  comment '异常',
   ExpUserData          text  comment '异常用户数据',
   Thread               varchar(100)  comment '线程ID',
   Location             varchar(500)  comment '方法名，源文件和行号
             警告：会影响性能。没有pdb文件的话，只有方法名，没有源文件名和行号。',
   HostName             varchar(100)  comment '主机名',
   HostIp               varchar(20)  comment '主机IP',
   Operator             varchar(50)  comment '操作者',
   ProjectId            varchar(100)  comment '程序标识',
   primary key (LogID)
);

alter table sys_log comment '系统级日志表，用于平台应用';
